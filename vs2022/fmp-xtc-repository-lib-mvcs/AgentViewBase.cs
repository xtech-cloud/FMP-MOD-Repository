...
namespace XTC.FMP.MOD.Repository.LIB.MVCS
{
    ...
    /// Agent视图层基类
    /// </summary>
    public class AgentViewBase : View
    {
        /// <summary>
        /// 带uid参数的构造函数
        /// </summary>
        ...
        public AgentViewBase(string _uid, string _gid) : base(_uid)
        {
            gid_ = _gid;
        }
        ...
        {
            var bridge = getFacade()?.getUiBridge() as IAgentUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// </summary>
        /// <param name="_err">错误</param>
        /// <param name="_dto">AgentRetrieveResponse的数据传输对象</param>
        public virtual void RefreshProtoRetrieve(Error _err, AgentRetrieveResponseDTO _dto, SynchronizationContext? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IAgentUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// </summary>
        /// <param name="_err">错误</param>
        /// <param name="_dto">AgentListResponse的数据传输对象</param>
        public virtual void RefreshProtoList(Error _err, AgentListResponseDTO _dto, SynchronizationContext? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IAgentUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// <param name="_err">错误</param>
        /// <param name="_dto">AgentListResponse的数据传输对象</param>
        public virtual void RefreshProtoSearch(Error _err, AgentListResponseDTO _dto, SynchronizationContext? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IAgentUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// </summary>
        /// <returns>服务层</returns>
        protected AgentService? getService()
        {
            if(null == service_)
                service_ = findService(AgentService.NAME + "." + gid_) as AgentService;
            return service_;
        }
        ...
        protected AgentFacade? getFacade()
        {
            if(null == facade_)
                facade_ = findFacade(AgentFacade.NAME + "." + gid_) as AgentFacade;
            return facade_;
        }
