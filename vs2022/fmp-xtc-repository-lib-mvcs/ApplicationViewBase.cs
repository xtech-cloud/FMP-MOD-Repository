...
namespace XTC.FMP.MOD.Repository.LIB.MVCS
{
    ...
    /// Application视图层基类
    /// </summary>
    public class ApplicationViewBase : View
    {
        /// <summary>
        /// 带uid参数的构造函数
        /// </summary>
        ...
        public ApplicationViewBase(string _uid, string _gid) : base(_uid)
        {
            gid_ = _gid;
        }
        ...
        {
            var bridge = getFacade()?.getUiBridge() as IApplicationUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// </summary>
        /// <param name="_err">错误</param>
        /// <param name="_dto">ApplicationRetrieveResponse的数据传输对象</param>
        public virtual void RefreshProtoRetrieve(Error _err, ApplicationRetrieveResponseDTO _dto, object? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IApplicationUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// </summary>
        /// <param name="_err">错误</param>
        /// <param name="_dto">ApplicationListResponse的数据传输对象</param>
        public virtual void RefreshProtoList(Error _err, ApplicationListResponseDTO _dto, object? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IApplicationUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        /// <param name="_err">错误</param>
        /// <param name="_dto">ApplicationListResponse的数据传输对象</param>
        public virtual void RefreshProtoSearch(Error _err, ApplicationListResponseDTO _dto, object? _context)
        {
            var bridge = getFacade()?.getUiBridge() as IApplicationUiBridge; 
            if (!Error.IsOK(_err))
            {
        ...
        {
            if(null == service_)
                service_ = findService(ApplicationService.NAME + "." + gid_) as ApplicationService;
            return service_;
        }
        ...
        {
            if(null == facade_)
                facade_ = findFacade(ApplicationFacade.NAME + "." + gid_) as ApplicationFacade;
            return facade_;
        }
        ...
        /// 直系服务层
        /// </summary>
        private ApplicationService? service_;

        /// <summary>
        /// 直系UI装饰层
        /// </summary>
        private ApplicationFacade? facade_;
    }
}
