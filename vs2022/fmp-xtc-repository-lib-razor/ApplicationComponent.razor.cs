...
namespace XTC.FMP.MOD.Repository.LIB.Razor
{
    public partial class ApplicationComponent
    {
        public class ApplicationUiBridge : IApplicationUiBridge
        {
            ...
            public ApplicationUiBridge(ApplicationComponent _razor)
            {
                razor_ = _razor;
            }
            ...
            {
                var dto = _dto as ApplicationRetrieveResponseDTO;
            }
            ...
            {
                var dto = _dto as ApplicationListResponseDTO;
                if (null == dto)
                    return;

                razor_.tableTotal = (int)dto.Value.Total;
                razor_.tableModel.Clear();
                foreach (var app in dto.Value.Applications)
                {
                    ...
                    razor_.tableModel.Add(new TableModel
                    {
                        ...
                        Size = Utility.SizeToString(app.File.Size),
                        Hash = app.File.Hash,
                        UpdatedAt = Utility.TimestampToString(app.UpdatedAt),
                        _Locked = Flags.HasFlag(app.Flags, Flags.LOCK),
                    });
                }
                razor_.StateHasChanged();
            }
            ...
            private ApplicationComponent razor_;
        }
        ...
        private async void onDownloadClick(string? _uuid)
        {
            if (string.IsNullOrEmpty(_uuid))
                return;

            var app = tableModel.Find((x) =>
            {
            ...
            if (null != jsRuntime_)
            {
                string uri = string.Format("http://minio.xtech.cloud/fmp.repository/applications/{0}/{1}@{2}/{3}/{0}.{1}.zip", app.Org, app.Name, app.Version, app.Platform);
        ...
        {
            searchLoading = true;
            var bridge = (getFacade()?.getViewBridge() as IApplicationViewBridge);
            if (null == bridge)
            {
                logger_?.Error("bridge is null");
                return;
            }
            var req = new ApplicationSearchRequest();
            ...
            var dto = new ApplicationSearchRequestDTO(req);
        ...
        private async void onCreateSubmit(EditContext _context)
        {
            ...
            req.Platform = model.Platform;
            ApplicationCreateRequestDTO dto = new ApplicationCreateRequestDTO(req);
        ...
        private async Task listAll()
        {
            ...
            var req = new ApplicationListRequest();
            req.Offset = (tablePageIndex - 1) * tablePageSize;
            req.Count = tablePageSize;
            var dto = new ApplicationListRequestDTO(req);
